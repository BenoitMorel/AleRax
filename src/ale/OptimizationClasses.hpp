 ...
 *  An optimization class is a set of pairs of (paramType, speciesNodeIndex) that
 *  share the same value when optimizing the model parameters. Each optimization
...
public:
   ...
   *  and vice versa. Applying both methods results in the same initial parameter vector.
   *
   *  -Full parameters: raw parameter vector, with one value for each species and each parameter
   *   type, the order of the elements is the same as in the AleModelParameters class.
...
private:
  // _classes[paramType][speciesNodeIndex] = integer representing the corresponding class
