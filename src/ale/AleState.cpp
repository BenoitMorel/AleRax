...
void AleState::serialize(const std::string &outputDir) const
{
  ...
  if (ParallelContext::getRank() == 0) {
    std::string checkpointPath = FileSystem::joinPaths(outputDir, "mainCheckpoint.txt");
  ...
  for (unsigned int f = 0; f < perFamilyModelParameters.size(); ++f) {
    ...
    std::string outputFilePath = FileSystem::joinPaths(outputDir, family + ".txt");
    ...
    const std::vector<std::string> &perCoreFamilyNames)
{
  ...
  for (auto &family: perCoreFamilyNames) {
    ...
    if (!is.good()) {
      Logger::error << "Can't open parameter checkpoint file " << paramsPath << std::endl;
      ParallelContext::abort(32);
    }
