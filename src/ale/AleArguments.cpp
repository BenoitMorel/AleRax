  ...
  optVerbose(false)
{
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "-h" || arg == "--help") {
      ...
      Logger::info << "\nError: --per-family-rates and --per-species-rates are deprecated and have been replaced "
                   << "with --model-parametrization PER-FAMILY or --model-paramtetrization PER-SPECIES\n" << std::endl;
      ParallelContext::abort(0);
    } else {
...
void AleArguments::printSummary() const
{
  ...
  case SpeciesTreeAlgorithm::User:
    Logger::info << "will be imported from the user file: " << speciesTree << std::endl;
    break;
  case SpeciesTreeAlgorithm::Random:
    Logger::info << "will be generated randomly" << std::endl;
    break;
  default:
    Logger::info << "will be generated with the method " << speciesTree << std::endl;
    break;
  }
#ifdef WITH_MPI
  ...
  case ModelParametrization::ORIGINATION_PER_SPECIES:
    Logger::info << "each species has a different set of origination probabilities, other rates are global" << std::endl;
    break;
  case ModelParametrization::CUSTOM:
    Logger::info << "clades of species sharing specific common rates are described in the user file: "
  ...
  case CCPRooting::ROOTED:
    Logger::info << "only the roots of the input gene trees are considered" << std::endl;
    break;
  case CCPRooting::MAD:
  ...
  case OriginationStrategy::OPTIMIZE:
    Logger::info << "gene families can originate from each species, "
                 << "the per-species origination probabilities will be estimated" << std::endl;
    break;
  }
  ...
  case SpeciesSearchStrategy::REROOT:
    Logger::info << "the starting species tree will be rerooted based on the gene tree distributions" << std::endl;
    break;
  case SpeciesSearchStrategy::HYBRID:
  case SpeciesSearchStrategy::SPR:
  case SpeciesSearchStrategy::TRANSFERS:
    Logger::info << "the optimal species tree topology will be inferred from the gene tree distributions" << std::endl;
    break;
  }
  if (inferSpeciationOrders) {
    Logger::info << "\tAleRax will estimate the relative order of speciation events from the inferred HGTs" << std::endl;
  }
  if (highways) {
    Logger::info << "\tAleRax will infer highways of horizontal gene transfer" << std::endl;
    if (highwayCandidateFile.size()) {
      Logger::info << "\tCandidate highways will be obtained from the user file: "
                   << highwayCandidateFile << std::endl;
    } else {
      Logger::info << "\tCandidate highways will be obtained from reconciliations, max candidates: "
                   << highwayCandidatesStep1 << std::endl;
    }
    Logger::info << "\tMax number of the best highways to retain after filtering the candidates: "
                 << highwayCandidatesStep2 << std::endl;
  }
  Logger::info << "\tNumber of reconciled gene trees to sample: " << geneTreeSamples << std::endl;
  Logger::info << "\tAleRax will exclude gene families covering less than " << minCoveredSpecies
               << " species" << std::endl;
  if (trimFamilyRatio != DEFAULT_TRIM_FAMILY_RATIO) {
    Logger::info << "\tAleRax will exclude top " << trimFamilyRatio * 100
                 << "\% of the gene families with the highest numbers of observed clades" << std::endl;
  }
  if (maxCladeSplitRatio != DEFAULT_MAX_SPLIT_RATIO) {
    Logger::info << "\tAleRax will exclude gene families with the observed clades to gene nodes ratio greater than "
                 << maxCladeSplitRatio << std::endl;
  }
  if (sampleFrequency != 1) {
    Logger::info << "\tAleRax will use only every i-th tree from the given gene tree distributions to obtain "
...
void AleArguments::printHelp() const
{
  ...
  Logger::info << "\t--speciation-gamma-categories <number of categories>" << std::endl;
  Logger::info << "\t--origination {UNIFORM, ROOT, LCA, OPTIMIZE}" << std::endl;
  Logger::info << "\t--rate-optimizer {LBFGSB, GRADIENT, SIMPLEX, GSL_SIMPLEX}" << std::endl;
...
void AleArguments::printWarning() const
{
  if (transferConstraint == TransferConstaint::NONE) {
    ...
    Logger::info << "! WARNING: --transfer-constraint NONE is biologically nonsensical and "
...
void AleArguments::checkValid() const
{
  ...
  {
    ok = false;
    Logger::info << "\nError: --model-parametrization ORIGINATION-PER-SPECIES should only be used "
